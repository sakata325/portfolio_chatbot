...
# import os # No longer needed as client uses env var implicitly
...
async def chat_endpoint(req: ChatRequest):
    if client is None:
        raise HTTPException(
            status_code=503,
            detail="Google GenAI クライアントが初期化されていません。設定を確認してください。"
        )
    ...
    try:
        ...
        if response.text:
            ...
            if block_reason:
                ...
                reply_text = f"応答が生成されませんでした (理由: {finish_reason})".strip()
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Google API エラーが発生しました: {e.message}"
        )
    except Exception as e:
